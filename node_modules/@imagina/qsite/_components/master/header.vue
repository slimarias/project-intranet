<template>
  <div id="masterHeader">
    ...
    <!-- MENU -->
    <q-drawer bordered id="menuMaster" class="no-shadow" v-model="drawer.menu"
              :mini="miniState" @click.capture="miniState = false">
      <!--Actions-->
      <div class="q-hide q-md-show">
        <!--Menu buttom-->
        <q-item clickable class="q-pa-none text-primary text-weight-bold" v-if="miniState">
          <!--Menu buttom-->
          <q-item-section avatar>
            <q-icon size="18px" color="grey-9" name="fas fa-bars"/>
          </q-item-section>
        </q-item>
        <!--Close buttom-->
        <div v-else class="text-right q-my-xs q-pr-sm">
          <q-btn dense round unelevated color="negative" icon="chevron_left" size="13px"
